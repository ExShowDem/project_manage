    ...
    <div class="portlet-title">
      ...
      <div class="actions" v-if="role_action.can_create">
        <a
          ...
          :href="route('admin.projects.invoices.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo hoá đơn mua vật tư
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              ...
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.invoices.edit', {projectId: currentProjectId, id: item.id})">
                  {{ item.contract_number }}
                </a>
                <a v-else :href="route('admin.projects.invoices.show', {projectId: currentProjectId, id: item.id})">
                  {{ item.contract_number }}
                </a>
              </td>
              <td>{{ item.supplier.name }}</td>
              <td>{{ item.contract_date }}</td>
              <td>{{ item.request ? item.request.code : 'Tự nhập' }}</td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.invoices.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a v-show="item.status !== 4"
                    ...
                    :href="route('admin.projects.invoices.receipt-input.create', {projectId: currentProjectId, id: item.id})">
                    <i class="fa fa-plus" />
                  </a>
                  <a v-if="role_action.can_update"
                     class="btn btn-xs btn-outline blue"
                     :class="(item.status === 3 && !role_action.is_admin) ? 'disabled' : ''"
                     :href="route('admin.projects.invoices.edit', {projectId: currentProjectId, id: item.id})">
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.invoices.index'), {
        params: params
      })
        .then((res) => {
          this.items = res.data;
    ...
    deleteItem(item) {
      return this.confirmAndDeleteItem(item, 'api.invoices.destroy')
    },
  }
};
</script>

    ...
    <div class="portlet-title">
      ...
      <div v-if="role_action.can_create" class="actions">
        <a
          ...
          :href="route('admin.projects.devices.input.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo nhập thiết bị
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              <td>{{ key + 1 }}</td>
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.devices.input.edit', {projectId: currentProjectId, input: item.id})">
                  {{ item.code }}
                </a>
                <a v-else :href="route('admin.projects.devices.input.show', {projectId: currentProjectId, input: item.id})">
                  {{ item.code }}
                </a>
              </td>
              <td>{{ item.company }}</td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.devices.input.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a
                    ...
                    :href="route('admin.projects.devices.input.edit', {projectId: currentProjectId, input: item.id})"
                  >
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.devices.input.index'), {
        params: params
      })
        .then((res) => {
          this.items       = res.data;
    ...
    deleteItem(id) {
      this.confirmDelete().then((result) => {
        if (result.value) {
          axios.delete(route('api.devices.input.destroy', id))
            .then(({code}) => {
              if (code === 0) {
                this.getItems();
              }

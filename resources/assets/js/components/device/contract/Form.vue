    ...
    <div class="portlet-title">
      <div v-if="id" class="caption">
        <div v-if="is_show">
          ...
          <span class="caption-subject font-green-haze bold uppercase">Xem hóa đơn mua thiết bị</span>
        </div>
        <div v-else>
          <i class="fa fa-pencil font-green-haze" />
          <span class="caption-subject font-green-haze bold uppercase">Sửa hóa đơn mua thiết bị</span>
        </div>
      </div>
      <div v-else class="caption">
        <i class="fa fa-plus font-green-haze" />
        <span class="caption-subject font-green-haze bold uppercase">Tạo hóa đơn mua thiết bị</span>
      </div>
    </div>
      ...
      <form action="#" class="form-horizontal form-plan">
        <div class="form-body">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label class="control-label col-md-3">Tên phiếu mua</label>
                <div class="col-md-9">
                  <select2 v-model="item.purchase_id" :settings="select2PurchaseOptions" :selected="selectedPurchase" @select="selectPurchase" :disabled="!allowUpdate" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="tab-content">
          ...
          <div id="tab_attach_file" class="tab-pane">
            <form-attach
              ...
              :model="{type: 'device-contract', id: this.id}"
            />
          </div>
        ...
        <div class="row margin-top-20">
          ...
          <div class="col-md-6 pull-right">
            <div class="pull-right">
              ...
              <a :href="route('admin.projects.devices.contract.index', currentProjectId)" class="btn default">
                Hủy
              </a>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
	  select2PurchaseOptions: this.getSelect2Settings({
	    url        : route('api.select2.device_purchases'),
	    field_name : 'name',
  ...
  mounted() {
    ...
    { // edit or show
      axios.get(route('api.devices.contract.show', this.id))
        .then((data) => {
          if (data.code === 0)
          {
            this.item        = data.data;
            ...
            this.selectedPurchase = {
              'id'  : this.item.purchase.id,
              'text': this.item.purchase.name,
            };
            this.item.purchase_id = this.item.purchase.id;
            ...
            this.item.devices.forEach((device) => {
              this.devices.push({
                ...
                code: device.code,
                unit: device.unit,
                needed_quantity: device.pivot.needed_quantity,
  ...
  methods: {
    ...
    save() 
    {
      ...
      this.item.devices.forEach((device) => {
        ...
        device.needed_quantity = device.needed_quantity ? parseFloat( device.needed_quantity.toString().replace(/[^\d.]/g, '') ) : 0;
      });

      if (this.id !== undefined) 
      {
        axios.put(route('api.devices.contract.update', this.id), this.item)
          .then((res) => {
            ...
            if (res.code == 0) 
            {
              this.$swal('', 'Sửa hóa đơn mua thiết bị thành công!', 'success').then(() => {
                window.location.href = route('admin.projects.devices.contract.index', this.currentProjectId);
              });
            }
      ...
      {//@todo DRY
        axios.post(route('api.devices.contract.store'), this.item)
          .then((res) => {
            ...
            if (res.code == 0) 
            {
              this.$swal('', 'Tạo hóa đơn mua thiết bị thành công!', 'success').then(() => {
                window.location.href = route('admin.projects.devices.contract.index', this.currentProjectId);
              });
            }
    ...
    downloadPdf() {
      downloadFile({
        url: route('api.export.device-contract.pdf'),
        method: 'POST',
        data: this.devices
      }, 'Hóa đơn mua thiết bị.pdf');
    },
    downloadXls() {
      downloadFile({
        url: route('api.export.device-contract.xls'),
        method: 'POST',
        data: this.devices
      }, 'Hóa đơn mua thiết bị.xlsx');
    },
    selectPurchase(purchase) 
    {
      ...
      {
        axios.get(route('api.devices.purchase.devices', purchase.id))
          .then((res) => {
            res.data.forEach((device) => {
              if (device.pivot.quantity > 0)
              {
                this.$refs.formDetail.items.push({
                  ...
                  code: device.code,
                  unit: device.unit,
                  needed_quantity: device.pivot.quantity,
                  quantity: 0,
                  unit_price: device.pivot.unit_price,
                  note: device.pivot.note,
                });
                //this.$refs.formDetail.selectedIds.push(device.id);
              }
            });
          });
      }
    ...
    openModelHistoryFile() {
      axios.get(route('api.files.history', {
        'fileable_id': this.id,
        'fileable_type': 'App\\Models\\DeviceContract'
      })).then(({ data }) => {

  ...
  <div class="portlet light bordered">
    <div class="portlet-title">
      <div class="caption">
        ...
        <div class="portlet-input input-inline input-small">
          <select2 :settings="select2SupplierOptions" v-model="searchOption.place" @select="getItems()" />
        </div>
        <div class="portlet-input input-inline input-small">
          <select2 :settings="select2PurchaseOptions" v-model="searchOption.purchase" @select="getItems()" />
        </div>
        <div class="portlet-input input-inline input-small">
          <div class="input-icon right">
      ...
      <div v-if="role_action.can_create" class="actions">
        <a
          ...
          :href="route('admin.projects.devices.contract.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo hóa đơn mua thiết bị
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              ...
              <td>{{ item.code }}</td>
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.devices.contract.edit', {projectId: currentProjectId, contract: item.id})">
                  {{ item.purchase.name }}
                </a>
                <a v-else :href="route('admin.projects.devices.contract.show', {projectId: currentProjectId, contract: item.id})">
                  {{ item.purchase.name }}
                </a>
              </td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.devices.contract.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a
                    ...
                    :href="route('admin.projects.devices.contract.edit', {projectId: currentProjectId, contract: item.id})"
                  >
...
export default {
  ...
  created() {
    ...
    this.select2SupplierOptions = this.getSelect2Settings({
      url: route('api.select2.suppliers'),
      field_name: 'name',
      placeholder: 'Chọn nơi mua...',
    });

    this.select2PurchaseOptions = this.getSelect2Settings({
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.devices.contract.index'), {
        params: params
      })
        .then((res) => {
          this.items       = res.data;
    ...
    deleteItem(id) {
      this.confirmDelete().then((result) => {
        if (result.value) {
          axios.delete(route('api.devices.contract.destroy', id))
            .then(({code}) => {
              if (code === 0) {
                this.getItems();
              }

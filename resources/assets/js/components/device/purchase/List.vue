  ...
  <div class="portlet light bordered">
    <div class="portlet-title">
      <div class="caption">
        ...
        <div class="portlet-input input-inline input-small">
          <select2 :settings="select2RequestOptions" v-model="searchOption.request" @select="getItems()" />
        </div>
        <div class="portlet-input input-inline input-small">
          <div class="input-icon right">
      ...
      <div v-if="role_action.can_create" class="actions">
        <a
          ...
          :href="route('admin.projects.devices.purchase.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo mua thiết bị
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              ...
              <td>{{ item.code }}</td>
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.devices.purchase.edit', {projectId: currentProjectId, purchase: item.id})">
                  {{ item.name }}
                </a>
                <a v-else :href="route('admin.projects.devices.purchase.show', {projectId: currentProjectId, purchase: item.id})">
                  {{ item.name }}
                </a>
              </td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.devices.purchase.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a
                    ...
                    :href="route('admin.projects.devices.purchase.edit', {projectId: currentProjectId, purchase: item.id})"
                  >
...
export default {
  ...
  created() {
    ...
    this.select2RequestOptions = this.getSelect2Settings({
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.devices.purchase.index'), {
        params: params
      })
        .then((res) => {
          this.items       = res.data;
    ...
    deleteItem(id) {
      this.confirmDelete().then((result) => {
        if (result.value) {
          axios.delete(route('api.devices.purchase.destroy', id))
            .then(({code}) => {
              if (code === 0) {
                this.getItems();
              }

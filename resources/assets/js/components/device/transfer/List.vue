  ...
  <div class="portlet light bordered">
    <div class="portlet-title">
      <div class="caption">
        ...
        <div class="portlet-input input-inline input-small">
          <select2 :settings="select2IssuanceOptions" v-model="searchOption.issuance" @select="getItems()" />
        </div>
        <div class="portlet-input input-inline input-small">
      ...
      <div v-if="role_action.can_create" class="actions">
        <a
          ...
          :href="route('admin.projects.devices.transfer.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo kế hoạch điều chuyển thiết bị
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              <td>{{ key }}</td>
              <td>{{ item.issuance.code }}</td>
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.devices.transfer.edit', {projectId: currentProjectId, transfer: item.id})">
                  {{ item.name }}
                </a>
                <a v-else :href="route('admin.projects.devices.transfer.show', {projectId: currentProjectId, transfer: item.id})">
                  {{ item.name }}
                </a>
              </td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.devices.transfer.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a
                    ...
                    :href="route('admin.projects.devices.transfer.edit', {projectId: currentProjectId, transfer: item.id})"
                  >
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.devices.transfer.index'), {
        params: params
      })
        .then((res) => {
          this.items       = res.data;
    ...
    deleteItem(id) {
      this.confirmDelete().then((result) => {
        if (result.value) {
          axios.delete(route('api.devices.transfer.destroy', id))
            .then(({code}) => {
              if (code === 0) {
                this.getItems();
              }

    ...
    <div class="portlet-title">
      <div class="caption">
        ...
        <div class="portlet-input input-inline input-small">
          ...
          <date-picker @dp-change="getItems()" v-model="searchOption.from_created_date" :config="datepickerOptions" />
          </div>
        </div>
        <div class="portlet-input input-inline input-small">
          <div class="input-icon right">
          <i class="fa fa-calendar" />
          <date-picker @dp-change="getItems()" v-model="searchOption.till_created_date" :config="datepickerOptions" />
          </div>
        </div>
        <div class="portlet-input input-inline input-small">
      ...
      <div v-if="role_action.can_create" class="actions">
        <a
          ...
          :href="route('admin.projects.devices.project.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo xuất thiết bị tới dự án
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              <td>{{ key + 1 }}</td>
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.devices.project.edit', {projectId: currentProjectId, project: item.id})">
                  {{ item.code }}
                </a>
                <a v-else :href="route('admin.projects.devices.project.show', {projectId: currentProjectId, project: item.id})">
                  {{ item.code }}
                </a>
              </td>
              <td>{{ item.company }}</td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.devices.project.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a
                    ...
                    :href="route('admin.projects.devices.project.edit', {projectId: currentProjectId, project: item.id})"
                  >
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.devices.project.index'), {
        params: params
      })
        .then((res) => {
          this.items       = res.data;
    ...
    deleteItem(id) {
      this.confirmDelete().then((result) => {
        if (result.value) {
          axios.delete(route('api.devices.project.destroy', id))
            .then(({code}) => {
              if (code === 0) {
                this.getItems();
              }

    ...
    <div class="portlet-title">
      ...
      <div v-if="role_action.can_create" class="actions">
        <a
          ...
          :href="route('admin.projects.devices.rental.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo cho thuê thiết bị
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          <thead>
            <tr>
              <th> Stt</th>                
              <th> Mã cho thuê</th>        
              <th> Tên phiếu cho thuê</th> 
              <th> Dự án</th>              
              <th> Loại đơn vị thuê</th>        
              <th> Đơn vị thuê</th>        
              <th> Người tạo</th>          
              <th> Tình trạng</th>         
              <th> Theo dõi</th>           
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              ...
              <td>{{ item.code }}</td>
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.devices.rental.edit', {projectId: currentProjectId, rental: item.id})">
                  {{ item.name }}
                </a>
                <a v-else :href="route('admin.projects.devices.rental.show', {projectId: currentProjectId, rental: item.id})">
                  {{ item.name }}
                </a>
              </td>
              <td>{{ item.project.name }}</td>
              <td>{{ item.borrower_type_name }}</td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.devices.rental.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a
                    ...
                    :href="route('admin.projects.devices.rental.edit', {projectId: currentProjectId, rental: item.id})"
                  >
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.devices.rental.index'), {
        params: params
      })
        .then((res) => {
          this.items       = res.data;
    ...
    deleteItem(id) {
      this.confirmDelete().then((result) => {
        if (result.value) {
          axios.delete(route('api.devices.rental.destroy', id))
            .then(({code}) => {
              if (code === 0) {
                this.getItems();
              }

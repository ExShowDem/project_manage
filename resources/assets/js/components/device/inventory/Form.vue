        ...
        <div class="row margin-top-20">
          <div class="col-md-6 pull-right">
            <div class="pull-right">
              ...
              <a :href="route('admin.projects.devices.inventory.index', currentProjectId)" class="btn default">
                Hủy
              </a>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    save() 
    {
      ...
      this.item.devices.forEach(function(item, index) {
        item.existing_quantity = item.quantity; 
      });

      if (this.id !== undefined) 
      {
        axios.put(route('api.devices.inventory.update', this.id), this.item)
          .then((res) => {
            ...
            if (res.code == 0) 
            {
              this.$swal('', 'Sửa kiểm kê thiết bị thành công!', 'success').then(() => {
                window.location.href = route('admin.projects.devices.inventory.index', this.currentProjectId);
              });
            }
      ...
      {//@todo DRY
        axios.post(route('api.devices.inventory.store'), this.item)
          .then((res) => {
            ...
            if (res.code == 0) 
            {
              this.$swal('', 'Tạo kiểm kê thiết bị thành công!', 'success').then(() => {
                window.location.href = route('admin.projects.devices.inventory.index', this.currentProjectId);
              });
            }

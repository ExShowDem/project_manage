    ...
    <div class="portlet-title">
      ...
      <div v-if="role_action.can_create" class="actions">
        <a
          ...
          :href="route('admin.projects.devices.inventory.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo kiểm kê thiết bị
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              ...
              <td>{{ item.code }}</td>
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.devices.inventory.edit', {projectId: currentProjectId, inventory: item.id})">
                  {{ item.name }}
                </a>
                <a v-else :href="route('admin.projects.devices.inventory.show', {projectId: currentProjectId, inventory: item.id})">
                  {{ item.name }}
                </a>
              </td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.devices.inventory.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a
                    ...
                    :href="route('admin.projects.devices.inventory.edit', {projectId: currentProjectId, inventory: item.id})"
                  >
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.devices.inventory.index'), {
        params: params
      })
        .then((res) => {
          this.items       = res.data;
    ...
    deleteItem(id) {
      this.confirmDelete().then((result) => {
        if (result.value) {
          axios.delete(route('api.devices.inventory.destroy', id))
            .then(({code}) => {
              if (code === 0) {
                this.getItems();
              }

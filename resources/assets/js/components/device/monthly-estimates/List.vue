    ...
    <div class="portlet-title">
      ...
      <div v-if="role_action.can_create" class="actions">
        <a
          ...
          :href="route('admin.projects.devices.monthly_estimates.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo dự trù thiết bị tháng
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              ...
              <td>{{ item.code }}</td>
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.devices.monthly_estimates.edit', {projectId: currentProjectId, monthly_estimate: item.id})">
                  {{ item.name }}
                </a>
                <a v-else :href="route('admin.projects.devices.monthly_estimates.show', {projectId: currentProjectId, monthly_estimate: item.id})">
                  {{ item.name }}
                </a>
              </td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.devices.monthly_estimates.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a
                    ...
                    :href="route('admin.projects.devices.monthly_estimates.edit', {projectId: currentProjectId, monthly_estimate: item.id})"
                  >
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.devices.monthly_estimates.index'), {
        params: params
      })
        .then((res) => {
          this.items       = res.data;
    ...
    deleteItem(id) {
      this.confirmDelete().then((result) => {
        if (result.value) {
          axios.delete(route('api.devices.monthly_estimates.destroy', id))
            .then(({code}) => {
              if (code === 0) {
                this.getItems();
              }

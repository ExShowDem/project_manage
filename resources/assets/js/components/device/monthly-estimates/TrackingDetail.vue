  ...
  <div class="portlet light bordered">
    ...
    <div class="portlet-title">
      <div class="caption">
        ...
        <span class="caption-subject font-green-haze bold uppercase">Lịch sử dự trù thiết bị tháng</span>
      </div>
    </div>
    ...
    axios.get(route('api.log.detail', this.log_id))
      .then((data) => {
        if (data.code === 0)
        {
          ...
          this.item.date = this.item.date ? moment(this.item.date).format(this.datepickerOptions.format) : '';

          this.item.devices.forEach((device) => {
            if (device.type == 1)
            {
              device.type_text = 'Cấp';
            }

            if (device.type == 2)
            {
              device.type_text = 'Trả';
            }
          });

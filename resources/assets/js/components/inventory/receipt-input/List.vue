  ...
  <div class="portlet light bordered">
    <div class="portlet-title">
      <div class="caption">
        ...
        <div class="portlet-input input-inline input-small">
          <select2 :settings="select2SupplierOptions" v-model="searchOption.supplier" @select="getItems()" />
        </div>
        <div class="portlet-input input-inline input-small">
      ...
      <div class="actions" v-if="role_action.can_create">
        <a
          ...
          :href="route('admin.projects.inventories.receipt-inputs.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo nhập kho
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              ...
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.inventories.receipt-inputs.edit', {projectId: currentProjectId, id: item.id})">
                  {{ item.code }}
                </a>
                <a v-else :href="route('admin.projects.inventories.receipt-inputs.show', {projectId: currentProjectId, id: item.id})">
                  {{ item.code }}
                </a>
              </td>
              ...
              <td>{{ item.request ? item.request.code : '' }}</td>
              <td>{{ item.reason }}</td>
              <td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.inventories.receipt-inputs.tracking', {projectId: currentProjectId, id: item.id})">
              ...
              <td>
                <div>
                  <a v-if="role_action.can_update"
                    ...
                    :href="route('admin.projects.inventories.receipt-inputs.edit', {projectId: currentProjectId, id: item.id})"
                  >
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.inventories.receipt-inputs.index'), {
        params: params
      })
        .then((res) => {
          this.items = res.data;
    ...
    deleteItem(item) {
      return this.confirmAndDeleteItem(item, 'api.inventories.receipt-inputs.destroy')
    },
  }
};
</script>

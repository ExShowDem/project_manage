  ...
  <div class="portlet light bordered">
    <div class="portlet-title">
      <div class="caption">
        ...
        <span class="caption-subject font-green-haze bold uppercase">Lịch sử chuyển kho</span>
      </div>
    </div>
    ...
    axios.get(route('api.log.detail', this.log_id))
      .then(async (res) => {
        if (res.code === 0)
        {
          ...
          this.selectedInput = {
            'id': this.item.input.id,
            'text': this.item.input.name,
          };

          this.selectedOutput = {
            'id': this.item.output.id,
            'text': this.item.output.name,
          };
          ...
          this.item.supplies.forEach((supply) => {
            this.supplies.push({
              ...
              quantity_input: supply.quantity_input,
              quantity_output: supply.quantity_output,

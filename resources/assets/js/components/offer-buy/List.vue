    ...
    <div class="portlet-title">
      ...
      <div class="actions" v-if="role_action.can_create">
        <a
          ...
          :href="route('admin.projects.offer-buys.create', currentProjectId)"
        >
          <i class="fa fa-plus" /> Tạo đề xuất mua vật tư
        </a>
      </div>
    </div>
        ...
        <table class="table table-hover">
          ...
          <tbody>
            <tr v-for="(item, key) in items.data" :key="key">
              ...
              <td>
                <a v-if="role_action.can_update" :href="route('admin.projects.offer-buys.edit', {projectId: currentProjectId, id: item.id})">
                  {{ item.name }}
                </a>
                <a v-else :href="route('admin.projects.offer-buys.show', {projectId: currentProjectId, id: item.id})">
                  {{ item.name }}
                </a>
              </td>
              ...
              <td>
                <a class="btn btn-xs btn-outline green"
                   :href="route('admin.projects.offer-buys.tracking', {projectId: currentProjectId, id: item.id})">
                  <i class="fa fa-search" />
                </a>
              </td>
              <td>
                <div>
                  <a v-show="item.status === 3" class="btn btn-xs btn-outline blue" :href="route('admin.projects.offer-buys.invoice.create', {projectId: currentProjectId, id: item.id})">
                    <i class="fa fa-plus" />
                  </a>
                  <a v-if="role_action.can_update"
                     class="btn btn-xs btn-outline blue"
                     :class="(item.status === 3 && !role_action.is_admin) ? 'disabled' : ''"
                     :href="route('admin.projects.offer-buys.edit', {projectId: currentProjectId, id: item.id})">
...
export default {
  ...
  methods: {
    getItems() {
      ...
      axios.get(route('api.offer-buys.index'), {
        params: params
      })
        .then((res) => {
          this.items = res.data;
    ...
    deleteItem(item) {
      return this.confirmAndDeleteItem(item, 'api.offer-buys.destroy')
    },
  }
};
</script>
